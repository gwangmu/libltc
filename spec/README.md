...
In [event notes](#Event), both URIs can be used as the _source path/address_ when creating a reference to an object (as a link) or embedding an image. Embedding images by directly specifying their local filesystem path is **supported but highly discouraged** (e.g., `![](/home/john/img.png)`), as this may result in broken and unrestorable image embedding due to a relatively subtle filesystem change.
