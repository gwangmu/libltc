...
In time tags, multiple whitespaces between fields are regarded as one whitespace, and only the whitespace between the date portion (`yyyy-mm-dd`) and the time portion (`hh:MM:ss`) is mandatory. An empty line above each time tag is ignored.
