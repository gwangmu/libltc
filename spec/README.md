...
An LTC object with a chart-local ID has both _a chart-local URI_ and _a web URI_. The chart-local URI equals its qualified ID. The web URI is a web address to an LTC file, combined with an HTML query key `id` at the end. For example, if `https://myltc.com/jone` is a web address to an LTC file, the web URI of the event object `e001` in such a file is `https://myltc.com/jone?id=e001`.

In [event notes](#Event), both URIs can be used as the _source path/address_ when creating a reference to an object (as a link) or embedding an image. By default, embedding images by directly specifying their local filesystem path is **unsupported** (e.g., `![](/home/john/img.png)`), but the front-end LTC tool may enable this with explicit user settings. Even when it's possible, this is **highly discouraged** because this may result in broken and unrestorable image embedding even by a relatively subtle filesystem change.
