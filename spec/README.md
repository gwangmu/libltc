...
In time tags, multiple whitespaces between fields are regarded as one whitespace, and the whitespaces before `Edit:` and `-->` are not mandatory. An empty line above each time tag is ignored.
